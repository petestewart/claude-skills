---
name: subagent
description: Execute a single ticket from PLAN.md with precision. You are a focused implementation agent—receive a ticket, implement it, validate it, report results, then stop. Works with the orchestrator skill.
...
- When spawned by the Orchestrator skill to work on a specific ticket
